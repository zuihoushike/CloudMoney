...
import name.hd.cloudmoney.widget.MixAudioDialog;
import name.hd.cloudmoney.widget.NetWorkInfoDialog;
...
//        }.start();
        ...
        flashBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if(mLSMediaCapture != null){
					...
					if(mFlashOn){
						flashBtn.setImageResource(R.mipmap.flashstop);
					}else {
						flashBtn.setImageResource(R.mipmap.flashstart);
					}
		...
		startPauseResumeBtn.setOnClickListener(new OnClickListener() {
			public void onClick(View v)
			{
				...
				if(!m_liveStreamingOn)
				{
					...
					mThread.start();
					startPauseResumeBtn.setImageResource(R.mipmap.stop);
				}else {
					showToast("停止直播中，请稍等。。。");
					stopAV();
					startPauseResumeBtn.setImageResource(R.mipmap.restart);
				}
			}
		});
	...
	private void addWaterMark(){
		if(mLSMediaCapture != null){
			Bitmap water = BitmapFactory.decodeResource(getResources(),R.mipmap.water);
			int x = 120;
			int y = 60;
